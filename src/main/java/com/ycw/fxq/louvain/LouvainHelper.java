...
public class LouvainHelper {
    ...
    public static void main(String[] args) throws IOException {
        ...
        a.init("F:/xczx/fxq/BankData/testdata.txt");
        a.louvain();
        double endTime = System.currentTimeMillis();
        writeOutputCluster("F:/xczx/fxq/BankData/cluster.txt",a);  //打印每个节点属于哪个簇
        writeOutputCircle("F:/xczx/fxq/BankData/circle.txt",a);   //打印每个簇有哪些节点
        System.out.format("program running time: %f seconds%n", (endTime - beginTime) / 1000.0);
    }

    public static void excute(String filePath, String fileName) throws IOException {
		Louvain a = new Louvain();
		a.init(filePath + fileName);
		a.louvain();
		writeOutputCluster(filePath + "cluster.txt", a); // 打印每个节点属于哪个簇
		writeOutputCircle(filePath + "circle.txt", a); // 打印每个簇有哪些节点
    }
